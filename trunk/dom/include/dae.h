...
template<class T> struct daeDocRoot : daeDocRef, daeOK
{
	...
	//REMOVE ME
	/*In conflict with unified daeSmartRef constructor/operator=.
...
{		
	 ...
	 * @see @c daeDOM::_addArchive() and see
	 * @c daeDOM::__daeDOM__new() for making
	 * a @c new allocated @c daeDOM meant to
	 * be assigned to @c daeSmartRef<daeDOM>.
	 */
...
COLLADA_(public) //ACCESSORS & MUTATORS
	...
	template<class _>
	/**WARNING
	 * @a _ is a dummy to force writing "<domAny>"; and reserved. 
	 * @warning @a NCName is not copied into any kind of special 
	 * memory. It should probably be 0-terminated, and permanent.
	 */
	inline daeDocRoot<_> newDoc(daeClientString2 NCName, const daeURI &URI, daeIOPlugin *I=nullptr)
	{
		daeDocRoot<_> o; _anyDoc(o,NCName,URI,I); return o;
	}
	/**TEMPLATE-SPECIALIZATION Creates/recreates a @c domAny rooted doc. */
	inline void _anyDoc(daeDocRoot<domAny> &o, daeClientString2 NCName, const daeURI &URI, daeIOPlugin *I)
	{ 
		o = newDoc(URI,I); daeDocumentRef docu = o->getDocument();
		daeDOM &DOM = *const_cast<daeDOM*>(getDOM());
		if(docu!=nullptr&&o==DAE_OK) (docu->getRoot()=domAnyRef(DOM))->getNCName() = NCName;
	}
...
COLLADA_(public) //daeSmartRef<T> T factories	
	...
	/** 
	 * @c getWorkingMemory_new<daeDocument>().
	 */
	NOALIAS_LINKAGE daeDocument *_addDocument(daeArchive*);
	/** 
	...
	/**HACK: Repurposing for compile-time-check below. */
	typedef void __DAEP__Object__signed;
	/**OVERLOAD Adds a generic object to the database. */
	template<class T> inline T *_add2(...)
	{
		typename T::__DAEP__Object__signed 
		Cannot_add_daeDOM;(void)Cannot_add_daeDOM;
	...
	/**OVERLOAD Adds an orphan, abstract element to the database. */
	template<class> inline daeElement *_add2(daeElement*)
	{
		return _addElement(domAny::_master_model());
	}
...
COLLADA_(public) //ACCESSORS & MUTATORS
	...
	/**WARNING, REPLACES @c daeURI &getBaseURI().
	...
	/**EXPERIMENTAL
	 * @see @c daeDOM::getWorkingMemory() Doxygentation.
	 */
	typedef const daeArray<const daeDoc*> WorkingMemory;
	/**EXPERIMENTAL
	 * @return Retuns the internal list of documents that are not
	 * bound to URI semantics, ostensibly for diagnostic purposes.
	 * This is supporting @c daeElement::getProvisionalDocument().
	 * @see @c getWorkingMemory_new(). 
	 * 
	 * @note It's not recommended to @c const_cast these pointers
	 * but it is safe to modify the documents this way. It is not
	 * different from @c daeElement::getDocument().
	 */
	NOALIAS_LINKAGE WorkingMemory &getWorkingMemory()const
	SNIPPET( return *(WorkingMemory*)&_provisionalDocs_etc; )

	template<class T> 
	/**WARNING
	 * @tparam T can be @c daeDocument.
	 *
	 * @warning These documents are recycled if they have no refs
	 * and empty URIs. @c daeElement::getProvisionalDocument() is
	 * used to make short-lived documents with transient elements.
	 * Usually those elements quickly leave the document derelict.
	 * Recylcling prevents their memory from leaking out.
	 *
	 * Think of this as a way to work with documents that is free
	 * of the DOM's URI based namespace. The URI fields can store
	 * anything without the possibility of collisions at the cost
	 * of not being able to locate them with normal URI resolvers.
	 * Currently docs are stuck for life in their modes and these
	 * are lmited to @c daeDocument objects.
	 *
	 * Manually creates a document not having URI semantics. Just
	 * like @c daeElement::getProvisionalDocument() returns.
	 * For these documents @c daeDoc::hasOwnURI() equals @c false.
	 * @returns A newly allocated @a T pointer on the back of the
	 * @c getWorkingMemory() pool.
	 */	
	inline daeSmartRef<T> getWorkingMemory_new()
	{
		return _addDocument(this); 
	}
...
COLLADA_(private) //INVISIBLE		
		...
		/**
		 * Host of @c daeElement::getProvisionalDocument().
		 * @see @c getWorkingMemory().
		 */
		daeArray<daeDocRef> _provisionalDocs_etc;
